...
## B. 아침 브리핑 자동 생성 (자녀 계정, 이미 설정됨)

평일 아침 7시(한국 시간)에 자녀의 Claude Code 루틴이 이 저장소를 열고 리포트를 만들어 `reports/오늘날짜.md` 로 저장합니다.
실행이 끝나면 자녀 계정 이메일로 요약 알림이 오고, 리포트 전문이 그 안에 들어 있습니다. 어머니께 그대로 전달(캡처, 카톡)하면 됩니다.

- 루틴 켜고 끄기, 시간 바꾸기: claude.ai/code 의 Routines 메뉴.
